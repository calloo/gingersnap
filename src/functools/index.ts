 ...
 */
export const all = R.curry(
  <T>(predicate: (v: T) => boolean | Future<boolean>, stream: Stream<T>): Future<boolean> =>
    stream
      .map(predicate)
 ...
 */
export const append = R.curry(<T>(value: T | Future<T>, stream: Stream<T>): Stream<T> => {
 ...
 */
export const prepend = R.curry(<T>(value: T | Future<T>, stream: Stream<T>): Stream<T> => {
  return Stream.of(async function* (signal: AbortSignal) {
    stream.cancelOnSignal(signal);
    if (value instanceof Future) {
      yield await value;
 ...
 */
export const concat = R.curry(<T>(value: Stream<T>, stream: Stream<T>): Stream<T> => {
  return Stream.of(async function* (signal: AbortSignal) {
...
export const filter = R.curry(
  <T>(predicate: (v: T) => boolean | Future<boolean>, stream: Stream<T>): Stream<T> => stream.filter(predicate)
);

export const map = R.curry(<T, V>(predicate: (v: T) => V | Future<V>, stream: Stream<T>) => stream.map(predicate));

export const insert = R.curry(
  <T>(index: number, value: T | Future<T>, stream: Stream<T>) =>
...
export const insertAll = R.curry(
  <T>(index: number, value: T[] | Array<Future<T>> | Stream<T>, stream: Stream<T>) =>
    Stream.of(async function* (signal: AbortSignal) {
      stream.cancelOnSignal(signal);
      let i = -1;
      let inserted = false;
...
export const slice = R.curry(<T>(start: number, end: number, stream: Stream<T>) =>
  stream.skip(start).take(end - start)
);

export const repeat = R.curry(
  <T>(value: Future<T>, count: number) =>
    Stream.of(async function* (signal: AbortSignal) {
...
export const join = R.curry((separator: string, stream: Stream<string>) => {
  let result = "";
  return stream
    .forEach((value) => {
...
export const without = R.curry(<T>(value: T[] | Future<T[]>, stream: Stream<T>) =>
...
export const repeatUntilSuccess = R.curry(
  <T>(retries: number, waitPeriodInSeconds: number, backoffFactor: number, functor: () => Future<T>) =>
...
export const nth = R.curry(<T>(index: number, stream: Stream<T>) => stream.skip(index).future);
